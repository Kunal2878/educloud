...
const AddStudentFees = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (data.isAdvancePayment && new Date(data.advancePaymentDate) > new Date()) {
    ...
    const feeData = {
      ...
      paymentDate: data.isAdvancePayment ? data.advancePaymentDate : null
    }
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-[32px] space-y-8 mb-[16px]">
            ...
            <div className="w-full sm:w-96 md:w-[24rem] lg:w-[28rem] mx-auto flex items-center">
              <input
                ...
                className="mr-2 w-4 h-4"
              />
              <label htmlFor="isAdvancePayment" className="text-black-300">Advance Payment</label>
            </div>
            {/* Advance Payment Date (conditionally rendered) */}
            {isAdvancePaymentChecked && (
              <div className="w-full sm:w-96 md:w-[24rem] lg:w-[28rem] mx-auto">
                <div className="relative">
                  <label className="block text-black-300 mb-1" htmlFor="advancePaymentDate">
                  ...
                  />
                  {errors.advancePaymentDate && (
                    <p className="text-red-500 text-sm mt-1">{errors.advancePaymentDate.message}</p>
                  )}
                </div>
              </div>
            )}
